...
namespace Level
{
    public class LevelGeneration : MonoBehaviour
    {
        ...
        private IsoTransform[,] transformMap;
        ...
        void Awake()
        {
            ...
            // TODO: Объединить map и tileMap
            //map = new TileTypes[(int)cellSizeX * gridDimensionsX + 6, (int)cellSizeY * gridDimensionsY + 6];
			map = new Tile[(int)cellSizeX * gridDimensionsX + 6, (int)cellSizeY * gridDimensionsY + 6];
            ...
            for (int i = 0; i != gridDimensionsX; i++)
            {
                for (int j = 0; j != gridDimensionsY; j++)
                {
                    for (int _i = 0; _i != cellSizeX; _i++)
                    {
                        ...
                        {
                            int room = grid[i, j];
                            Point pos = new Point(i * cellSizeX + _i, j * cellSizeY + _j);
							if (room >= 0)
							{
								map[pos.x,pos.y].type = rooms[room].GetBlock(_i, _j);
        ...
        void Draw()
        {
            for (int i = 0; i != map.GetLength(0); i++)
            {
                ...
                {
                    Transform bl = block;
					if (map[i, j].type!= TileTypes.empty)
                    {
						if (map[i, j].type == TileTypes.wall)
                        {
                            ...
                            if (i > 0)
                            {
                                if (map[i - 1, j].type == TileTypes.wall)
                                {
                                    mask += 2;
                                }
                            }
                            if (i < map.GetLength(0))
                            {
                                ...
                                else if (map[i + 1, j].type == TileTypes.empty)
                                {
                                    isTop = true;
                                }
                            }
                            if (j > 0)
                            {
								if (map[i, j - 1].type == TileTypes.wall)
                                {
                                    mask += 1;
                                }
                            }
                            if (j < map.GetLength(1))
                            {
                                ...
                                else if (map[i, j + 1].type == TileTypes.empty)
                                {
                                    isTop = true;
                                }
                            }
        ...
        void ConnectRooms(Room origin, Room target)
        {
            ...
            {
				if (n < map.GetLength(0) && map[n, roomY].type != TileTypes.ground)
                {
					map[n, roomY].type = TileTypes.ground;
					if (map[n, roomY + 1].type != TileTypes.ground)
                    {
						map[n, roomY + 1].type = TileTypes.ground;
                    }
					if (map[n, roomY + 2].type == TileTypes.empty)
                    {
						map[n, roomY + 2].type = TileTypes.wall;
                    }
					if (map[n, roomY - 1].type != TileTypes.ground)
                    {
						map[n, roomY - 1].type = TileTypes.ground;
            ...
            {
				if (map[targetX, j].type != TileTypes.ground)
                {
					map[targetX, j].type = TileTypes.ground;

					if (map[targetX + 1, j].type != TileTypes.ground)
                    {
						map[targetX + 1, j].type = TileTypes.ground;
                    }
					if (map[targetX + 2, j].type == TileTypes.empty)
                    {
						map[targetX + 2, j].type = TileTypes.wall;
                    }
					if (map[targetX - 1, j].type != TileTypes.ground)
                    {
						map[targetX - 1, j].type = TileTypes.ground;
